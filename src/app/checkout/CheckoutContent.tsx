...
const CheckoutContent = ({ userId ,accessToken,anonymousCartId}: WithAuthDetailProps) => {
  ...
  if (isAnonymousCartLoading || isLoading ) {
    return <>...</>;
  }

  console.log('aaaaaaaaaaaaaaaaa',anonymousCart)
  return (
    <div className="grid grid-cols-1 small:grid-cols-[1fr_416px] content-container gap-x-40 py-12">
      <PaymentWrapper cart={cart??anonymousCart}>
        { (cart||anonymousCart) &&<CheckoutForm cart={cart??anonymousCart} customer={customer} />}
      </PaymentWrapper>
      { (cart||anonymousCart)  && <CheckoutSummary cart={cart??anonymousCart} userId={userId}  accessToken={accessToken}/> }
    </div>
  );
};
