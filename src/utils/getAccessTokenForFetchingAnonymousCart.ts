...
export const getAccessTokenForFetchingAnonymousCart = async () => {
  try {
    const response = await axios.post(
      AUTH_URL,
      new URLSearchParams({
        grant_type: "client_credentials", // Correct grant type
        scope: `manage_orders:${PROJECT_KEY}`,
      }),
      {
        auth: {
          username: CLIENT_ID,
    ...
    console.error(
      "Error fetching access token for anonymous cart:",
